...
def error_matrix( reference, comparison, categories=None, unclassified=0 ):
    ...
    n = len( all_classes )
    combos = [x for x in itertools.product(all_classes, repeat=2)]
    em = np.array(
        [z.count(x) for z in list(zip(reference.flatten(), comparison.flatten())) for x in combos]
    ).reshape(n, n).view(ErrorMatrix)
    if categories:
...
class ErrorMatrix( np.ndarray ):
    ...
    def __array_finalize__(self, obj):
        ...
        if obj is None or not self.shape:
            return
        else:
            self.categories = list(getattr(obj, 'categories', range(1, 1+self.shape[0])))
